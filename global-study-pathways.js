...
(function(){
  ...
  function fmtLakh(value){
    ...
    if(lang==='gu') return '₹'+formatted+' લાખ';
    if(lang==='hi') return '₹'+formatted+' लाख';
  ...
  function fmtAUD(value){
    ...
    $('totalCost').textContent=fmtLakh(total);
    $('confirmedFunding').textContent=fmtLakh(funding);
    $('fundingGap').textContent=fmtLakh(gap);
    return {years,annual,total,funding,gap,partTime:num('partTime')};
  }

  function updateChecks(){
    const count=verifyChecks.filter(c=>c.checked).length;
    $('verifyCount').textContent=count+' / '+verifyChecks.length;
    const flags=redFlagChecks.filter(c=>c.checked).length;
    $('flagAlert').hidden=flags===0;
    ...
    const supportVisited=localStorage.getItem('gurushala-gsp-support-visited')==='1';
    if(supportVisited||num('governmentSupport')>0) completed++;
    const pct=Math.round((completed/4)*100);
    $('progressFill').style.width=pct+'%';
    $('progressCopy').textContent=completed+' / 4';
  }
  ...
  function restore(){
    try{
      ...
      if(data.values){
        Object.entries(data.values).forEach(([id,value])=>{if($(id)) $(id).value=value;});
      }
      if(typeof data.under18==='boolean') under18.checked=data.under18;
      if(typeof data.includeAustraliaVisaFee==='boolean' && $('includeAustraliaVisaFee')) $('includeAustraliaVisaFee').checked=data.includeAustraliaVisaFee;
  ...
  function handleChange(){
    minorNote.hidden=!under18.checked;
    syncAustraliaPanel();
  ...
  [...document.querySelectorAll('#government-support a')].forEach(link=>{
    link.addEventListener('click',()=>{
      localStorage.setItem('gurushala-gsp-support-visited','1');
      updateChecks();
    });
  });
  ...
  function renderSummary(scroll=true){
    ...
    let warning='';
    if(flags>0){
      warning='<div class="summary-warning"><strong>'+t('STOP AND VERIFY','STOP AND VERIFY','STOP AND VERIFY')+'</strong><br>'+t(
    ...
    const minor=under18.checked?'<div class="summary-warning"><strong>'+t('Under-18 check needed','Under-18 check needed','Under-18 check needed')+'</strong><br>'+t(
    ...
    output.innerHTML=
      ...
      '<div class="summary-grid">'+
        ...
        '<div><span>'+t('Funding gap','Funding gap','Funding gap')+'</span><strong>'+fmtLakh(b.gap)+'</strong></div>'+
        '<div><span>'+t('Verification checks','Verification checks','Verification checks')+'</span><strong>'+verified+' / '+verifyChecks.length+'</strong></div>'+
      '</div>'+
...
+n.toLocaleString('en-AU',{maximumFractionDigits:0});
  ...
  function australiaValues(){
    ...
    const annualLiving=basis?.value==='custom'?num('australiaCustomLiving'):parseFloat(basis?.value||'0');
    const housingWeekly=housingSelect?.value==='custom'?num('australiaCustomHousing'):parseFloat(housingSelect?.value||'0');
    const accommodationAnnual=Math.max(0,housingWeekly*52);
    const otherLiving=Math.max(0,annualLiving-accommodationAnnual);
    const visa=$('includeAustraliaVisaFee')?.checked?2500:0;
  ...
  function syncAustraliaPanel(){
    const panel=$('australiaEstimator');
    if(panel) panel.hidden=$('destination')?.value!=='Australia';
    const customLiving=$('australiaCustomLivingWrap');
    const customHousing=$('australiaCustomHousingWrap');
    if(customLiving) customLiving.hidden=$('australiaBasis')?.value!=='custom';
    if(customHousing) customHousing.hidden=$('australiaHousing')?.value!=='custom';
  }
  ...
  function renderAustraliaEstimate(){
    ...
    if(!a.annualLiving){
      output.innerHTML='<p>'+t('વાર્ષિક living-cost amount પસંદ કરો.','Choose an annual living-cost amount.','वार्षिक living-cost amount चुनें।')+'</p>';
      return;
    }
    ...
    output.innerHTML=
      ...
      (tooHigh?'<p class="estimate-warning">'+t('પસંદ કરેલું accommodation જ annual living benchmark કરતાં વધુ છે. આ એક મજબૂત સંકેત છે કે official minimum પર આધાર રાખવો પૂરતો નથી.','The selected accommodation alone exceeds the annual living benchmark. That is a strong sign the official minimum is not enough for this plan.','चुना गया accommodation अकेले annual living benchmark से अधिक है। यह संकेत है कि official minimum इस योजना के लिए पर्याप्त नहीं है।')+'</p>':'');
  }

  function applyAustraliaEstimate(){
    const a=australiaValues();
    if(a.rate<=0){
      window.alert(t('AUD → INR reference rate ઉમેરો.','Add an AUD → INR reference rate first.','पहले AUD → INR reference rate जोड़ें।'));
      return;
    }
    $('housing').value=(a.accommodationAnnual*a.rate/100000).toFixed(2);
    $('living').value=(a.otherLiving*a.rate/100000).toFixed(2);
    $('visaApplication').value=(a.visa*a.rate/100000).toFixed(2);
    renderBudget();
    save();
    renderAustraliaEstimate();
  }
  ...
  function renderBudget(){
    const years=Math.max(.5,num('duration')||0);
    const annual=num('tuition')+num('housing')+num('living')+num('health')+num('travel')+num('otherAnnual');
    const total=annual*years+num('setup');
    const funding=num('scholarship')+num('governmentSupport')+num('familyContribution')+num('approvedLoan');
    const gap=Math.max(0,total-funding);
    $('totalCost').textContent=fmtLakh(total);
    $('confirmedFunding').textContent=fmtLakh(funding);
